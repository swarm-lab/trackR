...
backgrounderQuanta <- function(video, n = 100, quant = c(0.025, 0.975), start = NULL, end = NULL) {
  ...
  shiny::showNotification("Calculating background. This may take a few minutes.",
                          id = "calc", duration = NULL)

  out <- lapply(quant, function(x) array(0.0, dim = c(nrow(video), ncol(video), 3)))

  for (i in 1:Rvision::nchan(l1[[1]])) {
    shiny::showNotification(paste0("Processing ", c("red", "green", "blue")[i], " channel."),
                            id = "layer", duration = NULL)
    tmp <- matrixStats::rowQuantiles(sapply(l1, function(x) x[][, , i]), probs = quant, drop = FALSE)
...
blobQuanta <- function(x, background, mask = NULL, smoothing, type) {
  if (type == "darker") {
    dif <- gaussianBlur(x, 11, 11, smoothing, smoothing) <
      gaussianBlur(background, 11, 11, smoothing, smoothing)
  } else {
    dif <- gaussianBlur(x, 11, 11, smoothing, smoothing) >
      gaussianBlur(background, 11, 11, smoothing, smoothing)
  }
...
pipelineQuanta <- function(video, begin, end, background, mask = NULL, smoothing,
  ...
  for (i in begin:end) {
    ...
    if (Rvision::isImage(local_mask)) {
      cc <- blobQuanta(frame, local_bg, local_mask, smoothing, type)
    } else {
      cc <- blobQuanta(frame, local_bg, NULL, smoothing, type)
    }
