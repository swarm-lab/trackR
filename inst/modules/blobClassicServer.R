...
observe({
  if (!is.null(input$blobThreshold) & !is.null(input$blobSize) &
    ...
    if (input$videoQuality < 1) {
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- blobClassic(resize(theImage(), fx = input$videoQuality,
                                   fy = input$videoQuality,
                                   interpolation = "area"),
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- blobClassic(theImage(), theBackground(), theMask(), input$blobBlur, input$blobThreshold)
        } else {
          cc <- blobClassic(theImage(), theBackground(), NULL, input$blobBlur, input$blobThreshold)
        }
      } else {
        cc$table <- data.frame()
      }
