...
output$blobBackgroundStatus <- renderUI({
...
observe({
  if (!is.null(input$blobSize) & !is.null(input$blobBlur) &
      isImage(theImage()) & theActive() == "blob") {
    if (input$videoQuality < 1) {
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- blobQuanta(resize(theImage(), fx = input$videoQuality,
                                  fy = input$videoQuality,
                                  interpolation = "area"),
                           ...
                           resize(theMask(), fx = input$videoQuality,
                                  fy = input$videoQuality,
                                  interpolation = "area"),
                           input$blobBlur, input$backgroundType)
        } else {
          cc <- blobQuanta(resize(theImage(), fx = input$videoQuality,
                                  fy = input$videoQuality,
                                  interpolation = "area"),
                           resize(theBackground(), fx = input$videoQuality,
                                  fy = input$videoQuality,
                                  interpolation = "area"),
                           NULL, input$blobBlur, input$backgroundType)
        }
      } else {
        cc$table <- data.frame()
      }
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- blobQuanta(theImage(), theBackground(), theMask(), input$blobBlur, input$backgroundType)
        } else {
          cc <- blobQuanta(theImage(), theBackground(), NULL, input$blobBlur, input$backgroundType)
        }
      } else {
        cc$table <- data.frame()
      }
