...
bookmarkExclude <- c(bookmarkExclude, "maskFile", "toggleMask")

# Toggle panel
observeEvent(input$toggleMask, {
  if ("maskPanel" %in% input$main) {
    updateCollapse(session, "main", close = "maskPanel")
  } else {
    updateCollapse(session, "main", open = "maskPanel")
    theActive("mask")
  }
})
...
observeEvent(input$maskFile, {
  ...
  if (nrow(path) > 0) {
    isolate({ theBookmarks$maskPath <- path$datapath })
  }
})

observeEvent(theBookmarks$maskPath, {
  toCheck <- tryCatch(Rvision::image(theBookmarks$maskPath),
                      error = function(e) NA)
