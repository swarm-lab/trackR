...
bookmarkExclude <- c(bookmarkExclude, "videoFile", "toggleVideo")

# Toggle panel
observeEvent(input$toggleVideo, {
  if ("videoPanel" %in% input$main) {
    updateCollapse(session, "main", close = "videoPanel")
  } else {
    updateCollapse(session, "main", open = "videoPanel")
    theActive("video")
  }
})
...
observeEvent(input$videoFile, {
  ...
  if (nrow(path) > 0) {
    isolate({ theBookmarks$videoPath <- path$datapath })
  }
})

observeEvent(theBookmarks$videoPath, {
  toCheck <- tryCatch(Rvision::video(theBookmarks$videoPath),
                      error = function(e) NA)
