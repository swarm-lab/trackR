  ...
  if (!is.null(input$blobThreshold) & !is.null(input$blobSize) &
    ...
    if (input$videoQuality < 1) {
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- trackR:::blobBGS(resize(theImage(), fx = input$videoQuality,
                                        fy = input$videoQuality,
                                        interpolation = "area"),
        ...
        if (isImage(theMask())) {
          cc <- trackR:::blobTracktor(resize(theImage(), fx = input$videoQuality,
                                             fy = input$videoQuality,
                                             interpolation = "area"),
                                      resize(theMask(), fx = input$videoQuality,
                                             fy = input$videoQuality,
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- trackR:::blobBGS(theImage(), theBackground(), theMask(), input$blobBlur, input$blobThreshold)
        } else {
          cc <- trackR:::blobBGS(theImage(), theBackground(), NULL, input$blobBlur, input$blobThreshold)
        }
      } else {
        if (isImage(theMask())) {
          cc <- trackR:::blobTracktor(theImage(), theMask(), input$blobBlur,
                                      input$blobThreshold)
        } else {
          cc <- trackR:::blobTracktor(theImage(), NULL, input$blobBlur,
                                      input$blobThreshold)
        }
      }
    }
