  ...
  if (!is.null(input$blobThreshold) & !is.null(input$blobSize) &
    ...
    if (input$videoQuality < 1) {
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- trackR:::blobQuanta(resize(theImage(), fx = input$videoQuality,
                                           fy = input$videoQuality,
                                           interpolation = "area"),
      ...
      if (isImage(theBackground())) {
        if (isImage(theMask())) {
          cc <- trackR:::blobQuanta(theImage(), theBackground(), theMask(), input$blobBlur, input$backgroundType)
        } else {
          cc <- trackR:::blobQuanta(theImage(), theBackground(), NULL, input$blobBlur, input$backgroundType)
        }
      } else {
        cc$table <- data.frame()
      }
