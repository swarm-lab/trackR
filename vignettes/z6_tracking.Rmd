...
`trackR` works its magic. 

Note: once the tracking is started, it cannot be interrupted without quitting 
the app and losing all your settings. This is unfortunately a consequence of the 
current functioning of [Shiny](https://github.com/rstudio/shiny/issues/1398)
that is not (yet) capable of cleanly stopping functions that are executing. 
I will implement a mechanism to save and restore your settings in case you need 
to interrupt the tracking before it ends (and hence have to close `trackR`), but 
this will not be implemented before at least the next version of `trackR`. 
